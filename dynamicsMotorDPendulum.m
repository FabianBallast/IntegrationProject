...
syms theta1 dtheta1 ddtheta1 theta2 dtheta2 ddtheta2 i_mot di_mot ddi_mot real;
syms m1 m2 k_gear J1 J2 l1 g c1 c2 b1 b2 R_mot L_mot k_mot V_in real positive;

q = [theta1; theta2; i_mot];
dq = [dtheta1; dtheta2; di_mot];
ddq = [ddtheta1; ddtheta2; ddi_mot];
...
T = 1/2 * vel' * mass * vel + 1/2 * L_mot * i_mot^2;
V = m1 * g * pos(2) + m2 * g * pos(5) - (V_in - k_mot*k_gear*dtheta1)*i_mot;
D = 1/2 * b1 * dtheta1^2 + 1/2 * b2 * dtheta2^2 + 1/2 * R_mot * i_mot^2;
...
sol_sim = zeros(5, length(t_sim));
sol_sim(:, 1) = [theta1_0; dtheta1_0; theta2_0; dtheta2_0; i_mot_0];
...
acc = matlabFunction(acc_sol,'Vars',[theta1; dtheta1; theta2; dtheta2; i_mot]);

% acc2 = @(th1, dth1, th2, dth2) acc()

for i = 2:length(t_sim)
    a = acc(sol_sim(1, i-1), sol_sim(2, i-1), sol_sim(3, i-1), sol_sim(4, i-1), sol_sim(5, i-1));
    sol_sim(:, i) = sol_sim(:, i-1) + step_size * [sol_sim(2, i-1); a(1); sol_sim(4, i-1); a(2); a(3)];
end
